...
function downloadSoundWithId(soundId, name) {
    return new Promise((resolve, reject) => {
        let file = fs.createWriteStream(`sounds/${name}`);
...
async function start() {
    ...
    if (options.forceSoundsDownload) {
        ...
        sounds.forEach((child) => {
            array.push({key: child.key, name: child.val()['name']});
            return false;
        })
        ;
        for (const [index, obj] of array.entries()) {
            console.log(`Downloaded: ${index + 1}/${array.length}`);
            await downloadSoundWithId(obj.key, obj.name);
        }
    }
